...
use blooke_os::{gdt::DOUBLE_FAULT_IST_INDEX, serial_print, exit_qemu, QemuExitCode, serial_println};
...
fn stack_overflow() {
    stack_overflow();
    volatile::Volatile::new(0).read(); // prevent the compiler from performing "tail call elimination"
}
